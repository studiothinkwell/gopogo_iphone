...
- (id)initWithSession:(FBSession*)session {
  if (self = [super initWithSession:session]) {
	  printf("FBloginDialog initwithSession");
...
- (void)dialogWillDisappear {
  ...
  [_getSessionRequest cancel];
	...
	if(![[NSUserDefaults standardUserDefaults] boolForKey:@"extend_permission_yes"]) 
	{
		printf("if no permission");
		FBPermissionDialog* dialog = [[[FBPermissionDialog alloc] init] autorelease];
				//  dialog.delegate = self;
		dialog.permission = @"email";//"status_update";
		[dialog show];
	...
	//else
//	{
//		printf("\n if basic permission is not allowed \n");
...
- (void)dialogDidSucceed:(NSURL*)url {
	didSucceed=1;
	printf("FBloginDialog dialogDidSucceed");
